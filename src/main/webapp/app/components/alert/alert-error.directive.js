(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('chepeyWebHipsterApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-chepeyWebHipsterApp-error');
                var entityKey = httpResponse.headers('X-chepeyWebHipsterApp-params');
                if (errorHeader) {
